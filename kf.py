...
CONFIG = {
    'C': 2,                 # コーホート数
    'fixed_mu': 2.0,        # インフレ定常値 (アンカー)
    'ages': [480, 720],     # 各コーホートの月齢 (例: 40歳, 60歳)
    ...
    # ファイルパス
    'mock_csv': 'mock_data_comprehensive.csv',
    'output_csv': 'endogenous_variables_comprehensive.csv',
...
# =====================================================================
# 4. Step 2: UKF (異質性スイッチ完全対応)
# =====================================================================
def fx(x, dt, phi):
    """状態遷移関数 (x[1:] にすることで gamma が何個でも RW として処理可能)"""
...
def hx(x, E_t, phi):
    """観測関数 (異質性フラグを見て参照する gamma を切り替える)"""
    pi_t = x[0]
    z = np.zeros(DIM_Z)
...
def setup_ukf(opt_phi, opt_sig_pi, opt_sig_S, sig_gamma, sig_Z):
    ...
    ukf.x = np.array([CONFIG['fixed_mu']] + [0.5] * (DIM_X - 1))
    
    # ★ P0のベストプラクティス: 状態変数ごとにスケールに見合った初期分散を与える
    # pi_t の初期標準偏差は 0.5%, 各 gamma の初期標準偏差は 0.2 とする
    ukf.P = np.diag([0.5**2] + [0.2**2] * (DIM_X - 1))
    
    # Q: プロセスノイズ分散 (全コーホート共通の sig_gamma を用いる)
    ukf.Q = np.diag([opt_sig_pi**2] + [sig_gamma**2] * (DIM_X - 1))
    # R: 観測ノイズ分散 (全コーホート共通の sig_Z を用いる)
...
if __name__ == "__main__":
    ...
    for t_short in range(CONFIG['T_short']):
        ...
        # 異質性フラグに応じた gamma の記録と各種変数の格納
        for c in range(CONFIG['C']):
            gamma_val = ukf_final.x[1 + c] if CONFIG['heterogeneous_gamma'] else ukf_final.x[1]
    ...
    if CONFIG['heterogeneous_gamma']:
        for c in range(CONFIG['C']):
            ...
            axes[1].plot(df_endogenous.index, df_endogenous[f'True_gamma_{c+1}'], color='black', linestyle=':', alpha=0.5) # 真値のプロット
        ...
        axes[1].plot(df_endogenous.index, df_endogenous['True_gamma_1'], color='black', linestyle=':', alpha=0.5)
    ...
    # パネル3: サーベイ vs インプライド vs 経験 (高齢コーホートの例)
    c_idx = CONFIG['C'] # 最も高齢のコーホートをプロット
    ...
    axes[2].set_title(f'Survey Expectations vs Implied (Cohort {c_idx}: {CONFIG["ages"][c_idx-1]} months)')
