...
def generate_mock_data():
    ...
    for c, age_years in enumerate(CONFIG['ages']):
        ...
        for t in range(start_idx, T_long):
            ...
            Z_star = true_gamma[t, c] * true_E_ct + (1 - true_gamma[t, c]) * true_rational_exp_1y
...
if __name__ == "__main__":
    ...
    for t_short in range(CONFIG['T_short']):
        ...
        # ★ マクロモデルからインプライされる 1年先 および 5年先 の合理的インフレ期待を計算
        ...
        record = {
            ...
            'State_pi_t': pi_t,
            'Rational_Exp_1y': rational_exp_1y, # 内生変数として出力: 1年先予想
            'Rational_Exp_5y': rational_exp_5y  # 内生変数として出力: 5年先予想
        }
        ...
        for c in range(CONFIG['C']):
            ...
            # ファンダメンタル予想の形成 (ここでは1年先の合理的期待を使用)
            Z_star = gamma_c * E_pool[t_short, c] + (1 - gamma_c) * rational_exp_1y
    ...
    axes[3].plot(df_endogenous.index, df_endogenous['Rational_Exp_1y'], color='teal', linestyle='--', label='Rational Expectation (1y ahead)')
