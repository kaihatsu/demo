...
def generate_mock_data():
    ...
    for c, age_years in enumerate(CONFIG['ages']):
        ...
        for t in range(start_idx, T_long):
            ...
            Z_data[t, c] = (1 - true_lambda[t, c]) * Z_star_1y + true_lambda[t, c] * Z_data[t-1, c] + np.random.normal(0, 0.15)
            
            # ファンダメンタル予想 (Z*) と 粘着情報のブレンド [5年先]
            Z_star_5y = true_gamma[t, c] * true_E_ct + (1 - true_gamma[t, c]) * true_rational_exp_5y
            Z5_data[t, c] = (1 - true_lambda[t, c]) * Z_star_5y + true_lambda[t, c] * Z5_data[t-1, c] + np.random.normal(0, 0.15)
...
# =====================================================================
def fx(x, dt, phi):
    """状態推移関数: 今月の見えない状態が、来月どう変化するかを定義します。"""
    x_next = np.empty_like(x)
    x_next[0] = (1 - phi) * CONFIG['fixed_mu'] + phi * x[0]
    # gamma_tilde と lambda_tilde はランダムウォーク
    x_next[1:] = x[1:] 
    return x_next

def hx(x, E_t, Z_lag, phi, abs_delta_pi, b_params, alpha):
    """非線形観測関数: 状態ベクトルから理論上の観測データ (1年先Zとプロキシ) を生成します。"""
    pi_t = x[0]
    z = np.zeros(DIM_Z)
    
    # サーベイ形成の基礎となる「1年先の合理的インフレ期待」
...
if __name__ == "__main__":
    ...
    for t_short in range(CONFIG['T_short']):
        ...
        for c in range(CONFIG['C']):
            ...
            # --- 【家計の5年先インフレ予想】の形成 ---
            # ※ インフレ経験 (E_pool) は過去の蓄積なので共通。合理的期待部分のみ 5y に置き換える
            ...
            record[f'Household_Exp_1y_{c+1}'] = Household_Exp_1y # 内生変数: 家計の1年先予想
            record[f'Household_Exp_5y_{c+1}'] = Household_Exp_5y # 内生変数: 家計の5年先予想
