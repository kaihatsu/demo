...
CONFIG = {
    ...
    # True : 世代ごとに異なる gamma_{c,t} を推計する (現実的なモデル)
    'heterogeneous_gamma': True, 
    
    # --- データ期間の設定 ---
    'start_date_long': '1954-04-01',  # 高齢層の過去の記憶を計算するため、十分に古い年月を指定
    'start_date_short': '2004-04-01', # アンケートデータが実際に存在する最初の月
    'end_date': '2024-03-01',         # データの終わりの月
    
    'calibrated_theta': 1.5,          # 記憶の減衰パラメータ (過去の経験をどれくらい忘れるか)
    
    # --- ファイル出力設定 ---
    'mock_csv': 'mock_data_final.csv',              # 生成した擬似データ
    'output_csv': 'endogenous_variables_final.csv', # 全・内生変数と1年先予想
...
CONFIG['init_step2'] = [0.05, 0.05, 0.1, 0.05, 1.0] + [0.1] * CONFIG['C']
CONFIG['bounds_step2'] = tuple(
    [(1e-4, 1.0), (1e-4, 1.0), (1e-4, 5.0), (1e-4, 1.0), (0.0, 10.0)] + 
    [(0.01, 0.99)] * CONFIG['C']
)

# --- 内部計算用の次元設定 ---
num_gamma_states = CONFIG['C'] if CONFIG['heterogeneous_gamma'] else 1
# 状態ベクトルX (見えない状態): [pi_t, gamma_1, gamma_2, lambda_1, lambda_2] (C=2なら計5次元)
DIM_X = 1 + num_gamma_states + CONFIG['C']
# 観測ベクトルZ (手元のデータ): [S_t, Z_1, Z_2, lambda_obs_1, lambda_obs_2] (C=2なら計5次元)
...
def generate_mock_data():
    ...
    # マクロインフレのボラティリティ (足元の変動幅) を計算
    abs_delta_pi = np.abs(np.diff(true_pi, prepend=true_pi[0]))
    ...
    for c, age_years in enumerate(CONFIG['ages']):
        ...
        Z_data[start_idx-1, c] = 2.0 # 後で Z_lag として使用するための初期値
        ...
        for t in range(start_idx, T_long):
            ...
            # ファンダメンタル予想 (Z*) と 粘着情報のブレンド
            Z_star = true_gamma[t, c] * true_E_ct + (1 - true_gamma[t, c]) * true_rational_exp
            Z_data[t, c] = (1 - true_lambda[t, c]) * Z_star + true_lambda[t, c] * Z_data[t-1, c] + np.random.normal(0, 0.15)
    ...
    return S_data, df_short['S_t'].values, df_short[[f'Z_{c+1}' for c in range(CONFIG['C'])]].values, \
           df_short[[f'Lambda_obs_{c+1}' for c in range(CONFIG['C'])]].values, Z_lag_short
...
def hx(x, E_t, Z_lag, phi, abs_delta_pi, b_params, alpha):
    ...
    # 1年先の合理的インフレ期待 (Rational Expectation)
    ...
    for c in range(CONFIG['C']):
        ...
        # --- (B) サーベイ予想 Z_{c,t} の生成 ---
        # インフレ経験と合理的期待のブレンドによるファンダメンタル予想 (Z*)
        ...
        # インフレ変動が大きいほど、偶然同じ選択肢に留まる確率が減衰する
...
def nll_step2(params, S_short_obs, Z_short_obs, lambda_obs_short, Z_lag_short, E_pool, abs_delta_pi_short, opt_phi, opt_sig_pi, opt_sig_S):
    """
    Step 2 の尤度関数
    ...
    for t in range(len(S_short_obs)):
        ...
        # 観測関数 hx の kwargs として推計中の alpha や b_params を渡す
...
if __name__ == "__main__":
    # データ準備
    S_data, S_short_obs, Z_short_obs, lambda_obs_short, Z_lag_short = generate_mock_data()
    ...
    # ロジスティックバイアス用のマクロボラティリティ (|Delta pi_t|) の計算
    abs_delta_pi_long = np.abs(np.diff(pi_hat_long, prepend=pi_hat_long[0]))
    ...
    res2 = minimize(nll_step2, CONFIG['init_step2'], 
                    args=(S_short_obs, Z_short_obs, lambda_obs_short, Z_lag_short, E_pool, abs_delta_pi_short, opt_phi, opt_sig_pi, opt_sig_S),
    ...
    for t_short in range(CONFIG['T_short']):
        ...
        ukf_final.update(z_t, E_t=E_pool[t_short], Z_lag=Z_lag_short[t_short], phi=opt_phi, 
                         abs_delta_pi=abs_delta_pi_short[t_short], b_params=opt_b, alpha=opt_alpha)
        total_log_likelihood += ukf_final.log_likelihood
        
        # 記録用の辞書作成 (コーホート共通変数)
        pi_t = ukf_final.x[0]
        rational_exp = (1 - opt_phi**12) * CONFIG['fixed_mu'] + (opt_phi**12) * pi_t
        record = {
            'Date': dates_short[t_short],
            'Obs_S': S_short_obs[t_short],
            'State_pi_t': pi_t,
            'Rational_Exp': rational_exp  # 1年先の合理的インフレ期待
        }
        ...
        for c in range(CONFIG['C']):
            ...
            stay_prob = opt_b[c] * (2.0 / (1.0 + np.exp(opt_alpha * abs_delta_pi_short[t_short])))
            expected_obs_lambda = lambda_c + (1 - lambda_c) * stay_prob
            Z_star = gamma_c * E_pool[t_short, c] + (1 - gamma_c) * rational_exp
            
            # λ に関連する一連の内生変数をすべて記録
            record[f'State_gamma_{c+1}'] = gamma_c
            record[f'State_lambda_{c+1}'] = lambda_c
            record[f'Fund_Z_star_{c+1}'] = Z_star
    ...
    # パネル3: 粘着情報 (lambda_{c,t}) とプロキシデータ
    for c in range(CONFIG['C']):
    ...
    # パネル4: サーベイ vs インプライド vs 経験
    c_idx = CONFIG['C'] # 最も高齢のコーホートのグラフを表示
    axes[3].plot(df_endogenous.index, df_endogenous[f'Obs_Z_{c_idx}'], label=f'Observed Survey $Z_{{{c_idx},t}}$', color='orange', alpha=0.4)
    axes[3].plot(df_endogenous.index, df_endogenous[f'Fund_Z_star_{c_idx}'], label=f'Model Implied Fundamental $Z^*_{{{c_idx},t}}$', color='darkorange', linewidth=2)
    axes[3].plot(df_endogenous.index, df_endogenous[f'Experience_E_{c_idx}'], label=f'MN Experience $E_{{{c_idx},t}}$', color='purple', linestyle='-', linewidth=2)
    axes[3].plot(df_endogenous.index, df_endogenous['Rational_Exp'], color='teal', linestyle='--', label='Rational Expectation (1y ahead)')
    axes[3].set_title(f'Fundamental Expectations vs Survey (Cohort {c_idx}: {CONFIG["ages"][c_idx-1]} years old)')
