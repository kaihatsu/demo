...
def save_all_parameters(config, res1_x, res2_x, metrics, output_filename):
    ...
    add_param('Evaluation', 'RMSE_Predictive', metrics['RMSE_Pred'], '予測力: 1期先予測誤差 (1-step ahead RMSE)')
    add_param('Evaluation', 'RMSE_Explanatory', metrics['RMSE_Expl'], '説明力: フィルタリング残差 (Filtered residual RMSE)')
...
def generate_mock_data():
    """歴史的ショック、時変の粘着情報、およびロジスティックバイアスを含むデータを生成しCSVに保存します"""
    np.random.seed(42)
...
def load_csv_data(csv_filepath):
    """
    保存されたCSVファイルからデータを読み込み、UKF推計に必要なタプルに変換します（本番環境用）
    """
...
def compute_exact_mn_experience(pi_hat_long, theta, ages, start_idx_short):
    """マクロ推論パスを用いて、各コーホートのインフレ経験 E_{c,t} を計算します"""
...
if __name__ == "__main__":
    # 1. テスト環境: モックデータを生成しCSVファイルに書き出す
    generate_mock_data()
    
    # 2. 本番環境: 保存されたCSVファイルから読み込む (実データへの移行時はここを変更)
    ...
    sum_sq_error_pred = 0.0  # 予測力 (Predictive: Z_{t|t-1})
    sum_sq_error_expl = 0.0  # 説明力 (Explanatory: Z_{t|t})
    count_Z = 0
    ...
    for t_short in range(CONFIG['T_short']):
        ...
        # ★ 評価指標: [予測力] Update前の「事前予測(Z_{t|t-1})」の乖離を計算
        ...
        for c in range(CONFIG['C']):
            ...
            # ★ 評価指標: [説明力] Update後の「フィルタリング推論値(Z_{t|t})」の乖離を計算
    ...
    print(f" RMSE (Predictive) : {rmse_pred:.4f} (1-step ahead prediction, Z_{{t|t-1}})")
    print(f" RMSE (Explanatory): {rmse_expl:.4f} (Filtered residual, Z_{{t|t}})")
    print(f"")
